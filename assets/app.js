...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  /* The Google review QR code is a static image generated at build time
   (tools/gen-review-qr.py -> assets/review-qr.svg) and served from our own
   domain. It used to be drawn by a library fetched from cdnjs, which is what
   blanked the homepage for the clinic owner on a slow connection: deferred
   scripts run in document order, so a stalled CDN request stopped this file
   running at all. Generating it up front removes the third-party script from
   the page entirely — one less thing to trust and one less thing to fail. */
});
